...
import { Box3, BoxGeometry, Group, Mesh, MeshBasicMaterial, AmbientLight, Object3D, Vector3 } from 'three'
...
// load 3d model.
space.load('./static/3d/scene3.glb').then(() => {
  ...
  rackTemperatureObject3DWrap = space.getObject3DWrapByFullName('Scene/datacenterglb/RootNode/room_main/B_5')
}).catch((err) => {
  console.error(err)
})

function checkIsRack (obj: Object3D): boolean {
  return checkNameIncludes(obj, 'A_') || checkNameIncludes(obj, 'B_')
}
...
const updateRightInfo = (() => {
  ...
  return (rackWrap: Object3DWrap|null, serverWrap?: Object3DWrap) => {
    if (rackWrap !== null) {
      ...
      rackNameEle.innerText = '设备' + rackWrap.object3D.name
      if (rackWrap !== oldRack) {
        updateChart2()
        oldRack = rackWrap
      }
  ...
  if (selectRackWrap !== null) {
    ...
    if (list.length > 0) {
      ...
      if (rack !== null) {
        ...
        updatePopoverContent('设备' + rack.name)
      }
    } else {
      boxHelperWrap.setVisible(false)
      updatePopoverContent(null)
